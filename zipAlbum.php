			...
			foreach ($files as $file) {
				...
				if (is_dir($file) === true) {
					...
					$a[] = array(
						...
						'source' => $src,
						'file' => $file,
						'size' => false != $size ? $size : 16000 // this will fallback in case no size
					);
				}
			}
		}
		...
		if ( isset( $foldercontent ) ) {
			foreach ($foldercontent as $entry) {
				...
				if ($t >= $maxsize) {
					$splits++;
					$t = 0;
					/* create lastdir in next archive, 
					in case files still exist even if the next file is not in this archive it doesn't hurt */
					if ($lastdir !== '') {
